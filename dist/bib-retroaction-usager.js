...
import { d as e, n as t, L as r, h as o, c as a, b as i, a as n, u as s } from "./lit-element-DgcYs_ed.js";
import { d as l, D as c, P as d, u as h } from "./unsafe-html-CYDD8bFW.js";
...
}, f = { attribute: true, type: String, converter: e, reflect: false, hasChanged: t }, g = (e2 = f, t2, r2) => {
  const { kind: o2, metadata: a2 } = r2;
  null == a2 && m("missing-class-metadata", `The class ${t2} is missing decorator metadata. This could mean that you're using a compiler that supports decorators but doesn't support decorator metadata, such as TypeScript 5.1. Please update your compiler.`);
...
function y(e2) {
  return (t2, r2) => "object" == typeof r2 ? g(e2, t2, r2) : b(e2, t2, r2);
}
/**
 * @license
 * Copyright 2017 Google LLC
 ...
 */
function x(e2) {
  return y({ ...e2, state: true, attribute: false });
}
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const _ = (e2, t2, r2) => (r2.configurable = true, r2.enumerable = true, Reflect.decorate && "object" != typeof t2 && Object.defineProperty(e2, t2, r2), r2);
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
...
function C(e2, t2) {
  return (r2, o2, a2) => _(r2, o2, { get() {
    return ((r3) => {
      const a3 = r3.renderRoot?.querySelector(e2) ?? null;
      if (null === a3 && t2 && !r3.hasUpdated) {
        const t3 = "object" == typeof o2 ? o2.name : o2;
        w("", `@query'd field ${JSON.stringify(String(t3))} with the 'cache' flag set for selector '${e2}' has been accessed before the first update and returned null. This is expected if the renderRoot tree has not been provided beforehand (e.g. via Declarative Shadow DOM). Therefore the value hasn't been cached.`);
      }
      return a3;
    })(this);
  } });
...
z.styles = [S], z = p([v("md-elevation")], z);
/**
 * @license
...
let T;
T = new MutationObserver((e2) => {
  for (const t2 of e2)
    t2.target[I]?.hostConnected();
});
class $ {
  get htmlFor() {
  ...
  constructor(e2, t2) {
    this.host = e2, this.onControlChange = t2, this.currentControl = null, e2.addController(this), e2[I] = this, T?.observe(e2, { attributeFilter: ["for"] });
  }
  attach(e2) {
 ...
 * SPDX-License-Identifier: Apache-2.0
 */
const E = ["focusin", "focusout", "pointerdown"];
class A extends r {
  constructor() {
    super(...arguments), this.visible = false, this.inward = false, this.attachableController = new $(this, this.onControlChange.bind(this));
  }
  get htmlFor() {
...
L.styles = [P], L = p([v("md-focus-ring")], L);
const O = l(
  /**
   * @license
  ...
  class extends c {
    ...
    update(e2, [t2]) {
      if (void 0 === this._previousClasses) {
        ...
        for (const e3 in t2)
          t2[e3] && !this._staticClasses?.has(e3) && this._previousClasses.add(e3);
        return this.render(t2);
      }
      const r2 = e2.element.classList;
      for (const e3 of this._previousClasses)
        e3 in t2 || (r2.remove(e3), this._previousClasses.delete(e3));
      for (const e3 in t2) {
        const o2 = !!t2[e3];
...
const F = ["click", "contextmenu", "pointercancel", "pointerdown", "pointerenter", "pointerleave", "pointerup"], M = window.matchMedia("(forced-colors: active)");
class q extends r {
  constructor() {
    super(...arguments), this.disabled = false, this.hovered = false, this.pressed = false, this.rippleSize = "", this.rippleScale = "", this.initialSize = 0, this.state = B.INACTIVE, this.checkBoundsAfterContextMenu = false, this.attachableController = new $(this, this.onControlChange.bind(this));
  }
  get htmlFor() {
    return this.attachableController.htmlFor;
  }
  ...
  render() {
    ...
    return o`<div class="surface ${O(e2)}"></div>`;
  }
  update(e2) {
  ...
  handlePointerleave(e2) {
    this.shouldReactToEvent(e2) && (this.hovered = false, this.state !== B.INACTIVE && this.endPressAnimation());
  }
  handlePointerup(e2) {
    if (this.shouldReactToEvent(e2)) {
      if (this.state !== B.HOLDING)
        return this.state === B.TOUCH_DELAY ? (this.state = B.WAITING_FOR_CLICK, void this.startPressAnimation(this.rippleStartEvent)) : void 0;
  ...
  async handlePointerdown(e2) {
    if (this.shouldReactToEvent(e2)) {
      if (this.rippleStartEvent = e2, !this.isTouch(e2))
        return this.state = B.WAITING_FOR_CLICK, void this.startPressAnimation(e2);
      this.checkBoundsAfterContextMenu && !this.inBounds(e2) || (this.checkBoundsAfterContextMenu = false, this.state = B.TOUCH_DELAY, await new Promise((e3) => {
        setTimeout(e3, 150);
      }), this.state === B.TOUCH_DELAY && (this.state = B.HOLDING, this.startPressAnimation(e2)));
    }
  }
  handleClick() {
    this.disabled || (this.state !== B.WAITING_FOR_CLICK ? this.state === B.INACTIVE && (this.startPressAnimation(), this.endPressAnimation()) : this.endPressAnimation());
  }
...
p([y({ type: Boolean, reflect: true })], q.prototype, "disabled", void 0), p([x()], q.prototype, "hovered", void 0), p([x()], q.prototype, "pressed", void 0), p([C(".surface")], q.prototype, "mdRoot", void 0);
/**
 * @license
 * Copyright 2024 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */
const N = a`:host{display:flex;margin:auto;pointer-events:none}:host([disabled]){display:none}@media(forced-colors: active){:host{display:none}}.surface,:host{border-radius:inherit;position:absolute;inset:0;overflow:hidden}.surface{-webkit-tap-highlight-color:transparent}.surface::after,.surface::before{content:"";opacity:0;position:absolute}.surface::before{background-color:var(--md-ripple-hover-color,var(--md-sys-color-on-surface,#1d1b20));inset:0;transition:opacity 15ms linear,background-color 15ms linear}.surface::after{background:radial-gradient(closest-side,var(--md-ripple-pressed-color,var(--md-sys-color-on-surface,#1d1b20)) max(100% - 70px,65%),transparent 100%);transform-origin:center center;transition:opacity 375ms linear}.hovered::before{background-color:var(--md-ripple-hover-color,var(--md-sys-color-on-surface,#1d1b20));opacity:var(--md-ripple-hover-opacity,.08)}.pressed::after{opacity:var(--md-ripple-pressed-opacity,.12);transition-duration:105ms}`;
let j = class extends q {
};
j.styles = [N], j = p([v("md-ripple")], j);
/**
 * @license
 * Copyright 2023 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */
const D = ["ariaAtomic", "ariaAutoComplete", "ariaBusy", "ariaChecked", "ariaColCount", "ariaColIndex", "ariaColSpan", "ariaCurrent", "ariaDisabled", "ariaExpanded", "ariaHasPopup", "ariaHidden", "ariaInvalid", "ariaKeyShortcuts", "ariaLabel", "ariaLevel", "ariaLive", "ariaModal", "ariaMultiLine", "ariaMultiSelectable", "ariaOrientation", "ariaPlaceholder", "ariaPosInSet", "ariaPressed", "ariaReadOnly", "ariaRequired", "ariaRoleDescription", "ariaRowCount", "ariaRowIndex", "ariaRowSpan", "ariaSelected", "ariaSetSize", "ariaSort", "ariaValueMax", "ariaValueMin", "ariaValueNow", "ariaValueText"];
function H(e2) {
...
function G(e2) {
  for (const t2 of D)
    e2.createProperty(t2, { attribute: H(t2), reflect: true });
 ...
 */
D.map(H);
const U = Symbol("internals"), V = Symbol("privateInternals");
function W(e2) {
...
function Y(e2) {
  e2.addInitializer((e3) => {
    ...
    t2.addEventListener("click", async (e4) => {
      const { type: r2, [U]: o2 } = t2, { form: a2 } = o2;
...
var J;
G(X), Y(X), X.formAssociated = true, X.shadowRootOptions = { mode: "open", delegatesFocus: true }, p([y({ type: Boolean, reflect: true })], X.prototype, "disabled", void 0), p([y()], X.prototype, "href", void 0), p([y()], X.prototype, "target", void 0), p([y({ type: Boolean, attribute: "trailing-icon", reflect: true })], X.prototype, "trailingIcon", void 0), p([y({ type: Boolean, attribute: "has-icon", reflect: true })], X.prototype, "hasIcon", void 0), p([y()], X.prototype, "type", void 0), p([y({ reflect: true })], X.prototype, "value", void 0), p([C(".button")], X.prototype, "buttonElement", void 0), p([(J = { slot: "icon", flatten: true }, (e2, t2) => {
  const { slot: r2, selector: o2 } = J ?? {}, a2 = "slot" + (r2 ? `[name=${r2}]` : ":not([name])");
  return _(e2, t2, { get() {
    const e3 = this.renderRoot?.querySelector(a2), t3 = e3?.assignedElements(J) ?? [];
...
oe.styles = [re, te, ee], oe = p([v("md-filled-button")], oe);
/**
 * @license
 * Copyright 2021 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 ...
 * SPDX-License-Identifier: Apache-2.0
 */
const ie = a`:host{--_container-height:var(--md-text-button-container-height, 40px);--_disabled-label-text-color:var(--md-text-button-disabled-label-text-color, var(--md-sys-color-on-surface, #1d1b20));--_disabled-label-text-opacity:var(--md-text-button-disabled-label-text-opacity, 0.38);--_focus-label-text-color:var(--md-text-button-focus-label-text-color, var(--md-sys-color-primary, #6750a4));--_hover-label-text-color:var(--md-text-button-hover-label-text-color, var(--md-sys-color-primary, #6750a4));--_hover-state-layer-color:var(--md-text-button-hover-state-layer-color, var(--md-sys-color-primary, #6750a4));--_hover-state-layer-opacity:var(--md-text-button-hover-state-layer-opacity, 0.08);--_label-text-color:var(--md-text-button-label-text-color, var(--md-sys-color-primary, #6750a4));--_label-text-font:var(--md-text-button-label-text-font, var(--md-sys-typescale-label-large-font, var(--md-ref-typeface-plain, Roboto)));--_label-text-line-height:var(--md-text-button-label-text-line-height, var(--md-sys-typescale-label-large-line-height, 1.25rem));--_label-text-size:var(--md-text-button-label-text-size, var(--md-sys-typescale-label-large-size, 0.875rem));--_label-text-weight:var(--md-text-button-label-text-weight, var(--md-sys-typescale-label-large-weight, var(--md-ref-typeface-weight-medium, 500)));--_pressed-label-text-color:var(--md-text-button-pressed-label-text-color, var(--md-sys-color-primary, #6750a4));--_pressed-state-layer-color:var(--md-text-button-pressed-state-layer-color, var(--md-sys-color-primary, #6750a4));--_pressed-state-layer-opacity:var(--md-text-button-pressed-state-layer-opacity, 0.12);--_disabled-icon-color:var(--md-text-button-disabled-icon-color, var(--md-sys-color-on-surface, #1d1b20));--_disabled-icon-opacity:var(--md-text-button-disabled-icon-opacity, 0.38);--_focus-icon-color:var(--md-text-button-focus-icon-color, var(--md-sys-color-primary, #6750a4));--_hover-icon-color:var(--md-text-button-hover-icon-color, var(--md-sys-color-primary, #6750a4));--_icon-color:var(--md-text-button-icon-color, var(--md-sys-color-primary, #6750a4));--_icon-size:var(--md-text-button-icon-size, 18px);--_pressed-icon-color:var(--md-text-button-pressed-icon-color, var(--md-sys-color-primary, #6750a4));--_container-shape-start-start:var(--md-text-button-container-shape-start-start, var(--md-text-button-container-shape, var(--md-sys-shape-corner-full, 9999px)));--_container-shape-start-end:var(--md-text-button-container-shape-start-end, var(--md-text-button-container-shape, var(--md-sys-shape-corner-full, 9999px)));--_container-shape-end-end:var(--md-text-button-container-shape-end-end, var(--md-text-button-container-shape, var(--md-sys-shape-corner-full, 9999px)));--_container-shape-end-start:var(--md-text-button-container-shape-end-start, var(--md-text-button-container-shape, var(--md-sys-shape-corner-full, 9999px)));--_leading-space:var(--md-text-button-leading-space, 12px);--_trailing-space:var(--md-text-button-trailing-space, 12px);--_with-leading-icon-leading-space:var(--md-text-button-with-leading-icon-leading-space, 12px);--_with-leading-icon-trailing-space:var(--md-text-button-with-leading-icon-trailing-space, 16px);--_with-trailing-icon-leading-space:var(--md-text-button-with-trailing-icon-leading-space, 16px);--_with-trailing-icon-trailing-space:var(--md-text-button-with-trailing-icon-trailing-space, 12px);--_container-color:none;--_disabled-container-color:none;--_disabled-container-opacity:0}`;
let ne = class extends ae {
};
ne.styles = [re, ie], ne = p([v("md-text-button")], ne);
/**
 * @license
 * Copyright 2020 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const se = Symbol.for(""), le = (e2) => {
  if (e2?.r === se)
    return e2?._$litStatic$;
}, ce = (e2, ...t2) => ({ _$litStatic$: t2.reduce((t3, r2, o2) => t3 + ((e3) => {
...
})(r2) + e2[o2 + 1], e2[0]), r: se }), de = /* @__PURE__ */ new Map(), he = (ue = o, (e2, ...t2) => {
  const r2 = t2.length;
  let o2, a2;
  const i2 = [], n2 = [];
  let s2, l2 = 0, c2 = false;
  for (; l2 < r2; ) {
    for (s2 = e2[l2]; l2 < r2 && void 0 !== (a2 = t2[l2], o2 = le(a2)); )
      s2 += o2 + e2[++l2], c2 = true;
    l2 !== r2 && n2.push(a2), i2.push(s2), l2++;
  }
  if (l2 === r2 && i2.push(e2[r2]), c2) {
    const r3 = i2.join("$$lit$$");
    void 0 === (e2 = de.get(r3)) && (i2.raw = i2, de.set(r3, e2 = i2)), t2 = n2;
  }
  return ue(e2, ...t2);
});
var ue;
...
class me extends ve {
  constructor() {
    super(...arguments), this.disabled = false, this.flipIconInRtl = false, this.href = "", this.target = "", this.ariaLabelSelected = "", this.toggle = false, this.selected = false, this.type = "submit", this.value = "", this.flipIcon = pe(this, this.flipIconInRtl);
  }
  get name() {
    return this.getAttribute("name") ?? "";
  }
  ...
  render() {
    const e2 = this.href ? ce`div` : ce`button`, { ariaLabel: t2, ariaHasPopup: r2, ariaExpanded: o2 } = this, a2 = t2 && this.ariaLabelSelected, i2 = this.toggle ? this.selected : n;
    let s2 = n;
    return this.href || (s2 = a2 && this.selected ? this.ariaLabelSelected : t2), he`<${e2}
  ...
  connectedCallback() {
    this.flipIcon = pe(this, this.flipIconInRtl), super.connectedCallback();
  }
...
ge.styles = [fe, be], ge = p([v("md-outlined-icon-button")], ge);
/**
 * @license
 * Copyright 2023 Google LLC
 * SPDX-License-Identifier: Apache-2.0
...
class ye extends r {
  ...
  render() {
    const { ariaLabel: e2 } = this;
    return o`<div class="progress ${O(this.getRenderClasses())}" role="progressbar" aria-label="${e2 || n}" aria-valuemin="0" aria-valuemax="${this.max}" aria-valuenow="${this.indeterminate ? n : this.value}">${this.renderIndicator()}</div>`;
  }
  getRenderClasses() {
    return { indeterminate: this.indeterminate, "four-color": this.fourColor };
  }
}
G(ye), p([y({ type: Number })], ye.prototype, "value", void 0), p([y({ type: Number })], ye.prototype, "max", void 0), p([y({ type: Boolean })], ye.prototype, "indeterminate", void 0), p([y({ type: Boolean, attribute: "four-color" })], ye.prototype, "fourColor", void 0);
/**
 * @license
 * Copyright 2023 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 ...
 * SPDX-License-Identifier: Apache-2.0
 */
const ke = a`:host{font-size:var(--md-icon-size,24px);width:var(--md-icon-size,24px);height:var(--md-icon-size,24px);color:inherit;font-variation-settings:inherit;font-weight:400;font-family:var(--md-icon-font,Material Symbols Outlined);display:inline-flex;font-style:normal;place-items:center;place-content:center;line-height:1;overflow:hidden;letter-spacing:normal;text-transform:none;user-select:none;white-space:nowrap;word-wrap:normal;flex-shrink:0;-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility;-moz-osx-font-smoothing:grayscale}::slotted(svg){fill:currentColor}::slotted(*){height:100%;width:100%}`;
let Se = class extends Ce {
};
Se.styles = [ke], Se = p([v("md-icon")], Se);
const _ze = class _ze extends r {
  constructor() {
    super();
...
t_fn = async function(e2) {
  ...
  return new Promise(async (e3, r3) => {
    this.state = _ze.STATES.SUBMITTING, t2.disabled = true, fetch(`${__privateGet(this, _e2)}/getguid`).then(async (t3) => {
      if (!t3.ok)
    ...
    const t3 = new u(e3);
    t3.vote = o2, t3.comment = r2.get("comment"), t3.email = r2.get("email"), await fetch(`${__privateGet(this, _e2)}/insertSubmission`, { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json" }, body: JSON.stringify(t3) }).catch((e4) => {
      throw new Error("Could not POST vote: ", e4);
    });
  }).then(() => {
    this.state = _ze.STATES.SUBMITTED;
  }).catch((e3) => {
    this.state = _ze.STATES.ERROR, console.error("Vote submission failed. ", e3);
  }).finally(() => {
    t2.disabled = false;
  });
};
_r = new WeakSet();
r_fn = function() {
  this._vote = null, this.state = _ze.STATES.INITIAL;
};
_o = new WeakSet();
o_fn = function() {
