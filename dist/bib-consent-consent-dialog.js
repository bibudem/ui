...
class a extends e {
  ...
  setConsentTokens(e2) {
    this.dispatchEvent(new CustomEvent("intern:change", { detail: e2 }));
  }
  show() {
  ...
  render() {
    return n`<bib-consent-dialog class="consent-dialog" ${r(this.dialogRef)}><div class="title" autofocus>L’UdeM reconnaît l’importance de respecter la vie privée</div><p>Vos données personnelles sont utilisées pour enregistrer vos préférences, votre consentement et produire des statistiques anonymes.<br>Pour plus d'informations, veuillez consulter notre <a href="https://vie-privee.umontreal.ca/confidentialite" target="_blank" rel="noopener noreferrer">politique de confidentialité</a> et nos <a href="https://vie-privee.umontreal.ca/conditions-dutilisation" target="_blank" rel="noopener noreferrer">conditions d’utilisation</a>.</p><div class="actions-container"><a href="#" @click="${() => this.showPreferences()}" class="consent-link">Personnaliser les témoins</a> <button class="btn--outlined" type="button" @click="${() => this.setConsentTokens(false)}">Tout refuser</button> <button class="btn--filled" type="button" @click="${() => this.setConsentTokens(true)}">Tout accepter</button></div></bib-consent-dialog>`;
  }
}
