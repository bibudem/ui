...
class n {
  constructor(e2, t2, s2) {
    if (this._$cssResult$ = true, s2 !== i) throw new Error("CSSResult is not constructable. Use `unsafeCSS` or `css` instead.");
...
const r = (e2) => new n("string" == typeof e2 ? e2 : String(e2), void 0, i), o = (e2, ...t2) => {
  const s2 = 1 === e2.length ? e2[0] : t2.reduce((t3, i2, s3) => t3 + ((e3) => {
    if (true === e3._$cssResult$) return e3.cssText;
...
class S extends HTMLElement {
  ...
  static getPropertyDescriptor(e2, t2, i2) {
    ...
    if (null == s2) {
      if ("value" in (h(this.prototype, e2) ?? {})) throw new Error(`Field ${JSON.stringify(String(e2))} on ${this.name} was declared as a reactive property but it's actually declared as a value on the prototype. Usually this is due to using @property or @state on a method.`);
  ...
  static finalizeStyles(e2) {
    ...
    if (Array.isArray(e2)) {
      ...
      for (const e3 of i2) t2.unshift(a(e3));
    } else void 0 !== e2 && t2.push(a(e2));
    return t2;
  }
  ...
  createRenderRoot() {
    ...
    return ((i3, s2) => {
      if (t) i3.adoptedStyleSheets = s2.map((e2) => e2 instanceof CSSStyleSheet ? e2 : e2.styleSheet);
  ...
  performUpdate() {
    ...
    if (e2 = { kind: "update" }, m.emitLitDebugLogEvents && m.dispatchEvent(new CustomEvent("lit-debug", { detail: e2 })), !this.hasUpdated) {
      this.renderRoot ??= this.createRenderRoot();
      {
        ...
        if (e4.length) throw new Error(`The following properties on element ${this.localName} will not trigger updates as expected because they are set using class fields: ${e4.join(", ")}. Native class fields and some compiled output will overwrite accessors used for detecting changes. See https://lit.dev/msg/class-field-shadowing for more information.`);
      }
      if (this.__instanceProperties) {
        for (const [e4, t3] of this.__instanceProperties) this[e4] = t3;
        this.__instanceProperties = void 0;
      }
      const e3 = this.constructor.elementProperties;
      if (e3.size > 0) for (const [t3, i3] of e3) true !== i3.wrapped || this._$changedProperties.has(t3) || void 0 === this[t3] || this._$changeProperty(t3, this[t3], i3);
    }
    let t2 = false;
  ...
  constructor({ strings: e2, _$litType$: t2 }, i2) {
    ...
    const o2 = e2.length - 1, a2 = this.parts, [l2, d2] = ((e3, t3) => {
      ...
      for (let t4 = 0; t4 < i3; t4++) {
        const i4 = e3[t4];
        let a3, l3, d3 = -1, h2 = 0;
        for (; h2 < i4.length && (o3.lastIndex = h2, l3 = o3.exec(i4), null !== l3); ) if (h2 = o3.lastIndex, o3 === H) {
          if ("!--" === l3[1]) o3 = j;
          else if (void 0 !== l3[1]) o3 = q;
          else if (void 0 !== l3[2]) Z.test(l3[2]) && (n3 = new RegExp(`</${l3[2]}`, "g")), o3 = B;
          else if (void 0 !== l3[3]) throw new Error("Bindings in tag names are not supported. Please use static templates instead. See https://lit.dev/docs/templates/expressions/#static-expressions");
        } else o3 === B ? ">" === l3[0] ? (o3 = n3 ?? H, d3 = -1) : void 0 === l3[1] ? d3 = -2 : (d3 = o3.lastIndex - l3[2].length, a3 = l3[1], o3 = void 0 === l3[3] ? B : '"' === l3[3] ? Y : J) : o3 === Y || o3 === J ? o3 = B : o3 === j || o3 === q ? o3 = H : (o3 = B, n3 = void 0);
      ...
      if (1 === s2.nodeType) {
        {
          ...
          if (/^(?:textarea|template)$/i.test(e3) && s2.innerHTML.includes(M)) {
            ...
            if ("template" === e3) throw new Error(t3);
            T("", t3);
          }
        }
        if (s2.hasAttributes()) for (const e3 of s2.getAttributeNames()) if (e3.endsWith(z)) {
        ...
        } else e3.startsWith(M) && (a2.push({ type: 6, index: n2 }), s2.removeAttribute(e3));
        if (Z.test(s2.tagName)) {
          const e3 = s2.textContent.split(M), t3 = e3.length - 1;
          if (t3 > 0) {
            s2.textContent = E ? E.emptyScript : "";
            for (let i3 = 0; i3 < t3; i3++) s2.append(e3[i3], D()), ee.nextNode(), a2.push({ type: 2, index: ++n2 });
            s2.append(e3[t3], D());
          }
        }
      } else if (8 === s2.nodeType) if (s2.data === A) a2.push({ type: 2, index: n2 });
      else {
        let e3 = -1;
        for (; -1 !== (e3 = s2.data.indexOf(M, e3 + 1)); ) a2.push({ type: 7, index: n2 }), e3 += M.length - 1;
      }
      n2++;
    }
    if (d2.length !== r2) throw new Error('Detected duplicate attribute bindings. This occurs if your template has duplicate attributes on an element tag. For example "<input ?disabled=${true} ?disabled=${false}>" contains a duplicate "disabled" attribute. The error was detected in the following template: \n`' + e2.join("${...}") + "`");
...
class re {
  ...
  _update(e2) {
    let t2 = 0;
    for (const i2 of this._$parts) void 0 !== i2 && (x && x({ kind: "set part", part: i2, value: e2[t2], valueIndex: t2, values: e2, templateInstance: this }), void 0 !== i2.strings ? (i2._$setValue(e2, i2, t2), t2 += i2.strings.length - 2) : i2._$setValue(e2[t2])), t2++;
  }
}
class oe {
  get _$isConnected() {
  ...
  _$setValue(e2, t2 = this) {
    if (null === this.parentNode) throw new Error("This `ChildPart` has no `parentNode` and therefore cannot accept a value. This likely means the element containing the part was manipulated in an unsupported way outside of Lit's control such that the part's marker nodes were ejected from DOM. For example, setting the element's `innerHTML` or `textContent` can do this.");
    if (e2 = ne(this, e2, t2), W(e2)) e2 === K || null == e2 || "" === e2 ? (this._$committedValue !== K && (x && x({ kind: "commit nothing to child", start: this._$startNode, end: this._$endNode, parent: this._$parent, options: this.options }), this._$clear()), this._$committedValue = K) : e2 !== this._$committedValue && e2 !== G && this._commitText(e2);
    else if (void 0 !== e2._$litType$) this._commitTemplateResult(e2);
    else if (void 0 !== e2.nodeType) {
      if (this.options?.host === e2) return this._commitText("[probable mistake: rendered a template's host in itself (commonly caused by writing ${this} in a template]"), void console.warn("Attempted to render the template host", e2, "inside itself. This is almost always a mistake, and in dev mode ", "we render some warning text. In production however, we'll ", "render it, which will usually result in an error, and sometimes ", "in the element disappearing from the DOM.");
      this._commitNode(e2);
    } else ((e3) => I(e3) || "function" == typeof e3?.[Symbol.iterator])(e2) ? this._commitIterable(e2) : this._commitText(e2);
  }
  _insert(e2) {
  ...
  _commitTemplateResult(e2) {
    ...
    if (this._$committedValue?._$template === s2) x && x({ kind: "template updating", template: s2, instance: this._$committedValue, parts: this._$committedValue._$parts, options: this.options, values: t2 }), this._$committedValue._update(t2);
    else {
  ...
  _commitIterable(e2) {
    ...
    let i2, s2 = 0;
    for (const n2 of e2) s2 === t2.length ? t2.push(i2 = new oe(this._insert(D()), this._insert(D()), this, this.options)) : i2 = t2[s2], i2._$setValue(n2), s2++;
  ...
  setConnected(e2) {
    if (void 0 !== this._$parent) throw new Error("part.setConnected() may only be called on a RootPart returned from render().");
...
class ae {
  ...
  _$setValue(e2, t2 = this, i2, s2) {
    ...
    let r2 = false;
    if (void 0 === n2) e2 = ne(this, e2, t2, 0), r2 = !W(e2) || e2 !== this._$committedValue && e2 !== G, r2 && (this._$committedValue = e2);
    else {
      const s3 = e2;
      let o2, a2;
      for (e2 = n2[0], o2 = 0; o2 < n2.length - 1; o2++) a2 = ne(this, s3[i2 + o2], t2, o2), a2 === G && (a2 = this._$committedValue[o2]), r2 ||= !W(a2) || a2 !== this._$committedValue[o2], a2 === K ? e2 = K : e2 !== K && (e2 += (a2 ?? "") + n2[o2 + 1]), this._$committedValue[o2] = a2;
    }
...
class he extends ae {
  constructor(e2, t2, i2, s2, n2) {
    if (super(e2, t2, i2, s2, n2), this.type = 5, void 0 !== this.strings) throw new Error(`A \`<${e2.localName}>\` has a \`@${t2}=...\` listener with invalid content. Event listeners in templates must have exactly one expression and no surrounding text.`);
  }
  _$setValue(e2, t2 = this) {
    if ((e2 = ne(this, e2, t2, 0) ?? K) === G) return;
...
const ue = (e2, t2, i2) => {
  if (null == t2) throw new TypeError(`The container to render into may not be ${t2}`);
...
let me;
ue.setSanitizer = (e2) => {
  if (te !== V) throw new Error("Attempted to overwrite existing lit-html security policy. setSanitizeDOMValueFactory should be called at most once.");
  te = e2;
...
//# sourceMappingURL=lit-element-DmnF17fR.js.map
