...
var _e, _t, _b_instances, s_fn, i_fn, n_fn, r_fn, o_fn;
...
import { a } from "./bib-CS0hTTok.js";
import { DB_STORE_NAME as c, DB_VERSION as l, DB_NAME as d } from "./constants3.js";
class b extends t {
  constructor() {
    super();
  ...
  _renderBoutonFermer() {
    return this.boutonFermer ? n`<button class="btn-close" aria-label="Fermer" @click="${__privateMethod(this, _b_instances, o_fn)}">${r('<svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24"><path d="M480-424 284-228q-11 11-28 11t-28-11q-11-11-11-28t11-28l196-196-196-196q-11-11-11-28t11-28q11-11 28-11t28 11l196 196 196-196q11-11 28-11t28 11q11 11 11 28t-11 28L536-480l196 196q11 11 11 28t-11 28q-11 11-28 11t-28-11L480-424Z"/></svg>')}</button>` : null;
  }
  render() {
    return __privateGet(this, _e)?.message ? n`<aside class="container"><div class="inner"><div class="message">${r(__privateGet(this, _e).message)}</div>${this._renderBoutonFermer()}</div></aside>` : null;
  }
  setMessage(e2) {
...
s_fn = function() {
  return new e(this, { task: async ([e2], { signal: t2 }) => {
    const s2 = new Promise(async (s3, i2) => {
      ...
      i2(new Error("The service responded with a message with a prop succes at", o2));
    });
    try {
      const e3 = await s2;
...
i_fn = async function(e2) {
  if (!e2 || !e2.message) return void this.setMessage(null);
  const { id: t2, message: s2 } = e2;
  if (!s2) return void this.setMessage(null);
  if (!("indexedDB" in window)) return void this.setMessage(s2);
  const i2 = __privateSet(this, _t, await o(d, l, { upgrade(e3) {
    e3.objectStoreNames.contains(c) || e3.createObjectStore(c);
  } }));
  try {
    const s3 = t2 || await async function(e3) {
      const t3 = new TextEncoder().encode(JSON.stringify(e3)), s4 = await crypto.subtle.digest("SHA-256", t3);
      return Array.from(new Uint8Array(s4)).map((e4) => e4.toString(16).padStart(2, "0")).join("");
    }(e2), n2 = await i2.get(c, s3);
    n2 ? n2.hidden || (await i2.delete(c, s3), __privateMethod(this, _b_instances, n_fn).call(this, { ...n2, id: s3 })) : __privateMethod(this, _b_instances, n_fn).call(this, { ...e2, id: s3 });
  } catch (e3) {
    console.error("Something went wrong with indexedDB: %o", e3), this.setMessage(s2);
  }
};
n_fn = async function(e2) {
  !this.dispatchEvent(new CustomEvent("bib:show", { bubbles: true, cancelable: true })) || (this.setMessage(e2), __privateGet(this, _t) && e2.id && await __privateGet(this, _t).put(c, { ...e2, hidden: false }, e2.id));
};
r_fn = async function() {
